...
const THEME_NAME = 'fictional-university-block-theme';
...
const blocks = ['banner', 'header', 'button'];
		...
		entry: function () {
			...
			if (blocks.length > 0) {
				blocks.forEach((block) => {
					const blockOutput = snakeToCamel(block);
					entries[
						blockOutput
					] = `.${THEME_DIR}/theme-blocks/${block}.jsx`;
				});
			}
			return entries;
		},
